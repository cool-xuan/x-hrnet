...
class HSUSA(nn.Module):
    ...
    def __init__(self,
        ...
        self.mean_conv = ConvModule(
            ...
            conv_cfg=dict(type='Conv2d'),
            act_cfg=None)
        self.group_weight = nn.Parameter(torch.ones([1, 1, 96, 1]), requires_grad=True)

        self.ln_enable = ln_enable
        if ln_enable:
    ...
    def forward(self, x: torch.Tensor):
        ...
        h, w = x.shape[2:]
        xq = self.mean_conv((x * self.group_weight[:, :, :h]).mean(-2, keepdim=True)) # n,c,1,w
        xq = xq.transpose(2, 3).softmax(-2) # n,c,w,1
        xv = x # n,c,h,w

        atten = torch.matmul(xv, xq) # n,c,h,1
        atten = self.conv(atten)
        if self.ln_enable:
            atten = self.ln(atten.transpose(1, -1)).transpose(1, -1)
            atten = self.sigmoid(atten)

        return x * atten


class WSUSA(nn.Module):
    ...
    def __init__(self,
        ...
        self.mean_conv = ConvModule(
            ...
            conv_cfg=dict(type='Conv2d'),
            act_cfg=None)
        self.group_weight = nn.Parameter(torch.ones([1, 1, 1, 72]), requires_grad=True)
        self.ln_enable = ln_enable
        if ln_enable:
            self.ln = nn.LayerNorm(channels)
            self.sigmoid = nn.Sigmoid()
    ...
    def forward(self, x: torch.Tensor):
        ...
        h, w = x.shape[2:]
        xq = self.mean_conv((x * self.group_weight[:, :, :, :w]).mean(-1, keepdim=True)) # n,c,h,1
        xq = xq.softmax(-2) # n,c,h,1
        xv = x.transpose(2, 3) # n,c,w,h

        atten = torch.matmul(xv, xq).transpose(2, 3) # n,c,w,1
        atten = self.conv(atten)
        if self.ln_enable:
            atten = self.ln(atten.transpose(1, -1)).transpose(1, -1)
            atten = self.sigmoid(atten)
...
class HWSUSA(nn.Module):
    ...
    def __init__(self,
                 ...
                 hw_shuffle=False):
        super().__init__()
        assert mode in ['hw', 'wh', None]

        self.mode = mode
        self.hw_shuffle = hw_shuffle
        self.shuffle_groups = 4
        if mode == 'hw':
            h_channels = channels // 4 * 3
            w_channels = channels // 4
        elif mode == 'wh':
            h_channels = channels // 4
            w_channels = channels // 4 * 3
        else:
            h_channels = w_channels = channels // 2
            self.shuffle_groups = 2
        self.h_channels = h_channels
        self.w_channels = w_channels

        self.hconv = HSUSA(self.h_channels, ln_enable)
        self.wconv = WSUSA(self.w_channels, ln_enable)

    def forward(self, x):
        x1, x2 = x.split([self.h_channels, self.w_channels], dim=1)
        out = torch.cat([self.hconv(x1), self.wconv(x2)], dim=1)
        if self.hw_shuffle:
...
class SXShuffleUnit(nn.Module):
    ...
    def __init__(self,
        ...
        self.branch2 = nn.Sequential(
            WSUSA(branch_features, ln_enable=True),
            ConvModule(
                branch_features,
                branch_features,
                kernel_size=3,
                ...
                norm_cfg=norm_cfg,
                act_cfg=None), 
            HSUSA(branch_features, ln_enable=True))
...
class LiteHRModule(nn.Module):
    ...
    def __init__(
        ...
        if module_type == 'naive':
            ...
            print('Not support')
            exit()
        self.layers = self._make_branches(num_branches, num_blocks, block)
        if self.with_fuse:
    ...
    def _make_one_branch(self, branch_index, num_blocks, block):
        """Make one branch."""
        layers = []
    ...
    def _make_branches(self, num_branches, num_blocks, block):
        """Make branches."""
        branches = []

        for i in range(num_branches):
            branches.append(self._make_one_branch(i, num_blocks, block))
